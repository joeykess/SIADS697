...
def update_ticker(ticker_filter,ticker,btn1,btn2,btn3,btn4,ma_filters):
    ...
    if ticker_filter == 'Ticker':
        ...
        title_text = 'Average Closing Price'

    fig.update_layout(title=dict(text=f'{ticker} {title_text}',font = dict(size = 20, color = 'white'), x = 0.5, y = 0.96),
...
def update_dropdown(filter_option):
    if filter_option == 'Ticker':
        col_labels = [{'label' :k, 'value' :k} for k in sorted(stock_df.ticker.unique())]
        return col_labels
    elif filter_option == 'Sector':
        col_labels = [{'label' :k, 'value' :k} for k in sorted(stock_df.sector.unique())]
        return col_labels
